...
const FSQuiz: React.FC = () => {
    ...
    const processFrame = async () => {
      ...
      if (isRecordingRef.current) {
        ...
        const detectedGesture =
          results.gestures[0]?.[0]?.['categoryName'] || 'No match found';
        // Ignore other than A to Z. (J and Z are also ignored.)
        updatePredicts(
          /^[A-Z]$/.test(detectedGesture) ? detectedGesture : 'No match found'
        );
      }
  ...
  const handleNextButtonClick = () => {
    if (quizNumber === NUM_OF_QUIZ - 1) {
      console.log('quizCharacterList', quizCharacterList);
      const score = quizCharacterList.filter(
        (item: QuizItem) => item.isCorrect
      ).length;
      const payload = {
        user_id: localStorage.getItem('userId'), // Hardcoded user ID as "1"
        score: score, // Calculated score
        quiz_data: quizCharacterList, // Full quiz data from quizCharacterList
      };
      ...
      fetch('http://127.0.0.1:5000/api/score', {
        ...
        .then((response) => {
          if (!response.ok) {
            ...
            return; // Exit the chain if response is not OK
          }
        ...
        .then((result) => {
          ...
          navigate('/quiz-result', {
            state: { selectedWordList: quizCharacterList },
          });
        })
        .catch((error) => {
          console.error('Error calling /api/score:', error);
          navigate('/quiz-result', {
            state: { selectedWordList: quizCharacterList },
          });
        });
  ...
  return (
    ...
    >
      ...
      <Card
        sx={{ padding: '1rem', boxShadow: '4px 4px 6px rgba(0, 0, 0, 0.3)' }}
      >
      ...
      <Typography variant="h6" color="textSecondary">
        Answer in sign language to the camera:
      </Typography>
      <Box>
        <video
          ref={videoRef}
      ...
      >
        ...
        >
          {answeredSign &&
            (result ? (
              <CheckCircleIcon
                sx={{ color: 'success.main', marginRight: '0.5rem' }}
              />
            ) : (
